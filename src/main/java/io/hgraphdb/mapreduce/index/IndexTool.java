...
/**
 * An abstract MR job to update an index.
 *
...
public abstract class IndexTool extends Configured implements Tool {
    ...
    protected abstract void setup(final HBaseGraph graph, final IndexMetadata index);

    protected abstract void cleanup(final HBaseGraph graph, final IndexMetadata index);

    protected abstract Class<? extends Mapper> getDirectMapperClass();

    protected abstract Class<? extends Reducer> getDirectReducerClass();

    protected abstract Class<? extends Mapper> getBulkMapperClass();

    protected abstract TableName getInputTableName(final HBaseGraph graph, final IndexMetadata index);

    protected abstract TableName getOutputTableName(final HBaseGraph graph, final IndexMetadata index);
    ...
    public int run(String[] args) throws Exception {
        ...
        try {
            ...
            configuration.set(Constants.MAPREDUCE_INDEX_TYPE, indexType.toString());
            configuration.set(Constants.MAPREDUCE_INDEX_LABEL, label);
            configuration.set(Constants.MAPREDUCE_INDEX_PROPERTY_KEY, propertyKey);
            ...
            IndexMetadata index = graph.getIndex(OperationType.WRITE, indexType, label, propertyKey);
            if (index == null) {
            ...
            TableName inputTableName = getInputTableName(graph, index);
            TableName outputTableName = getOutputTableName(graph, index);
            ...
            job.getConfiguration().set(TableInputFormat.SCAN, convertScanToString(getInputScan(graph, index)));
